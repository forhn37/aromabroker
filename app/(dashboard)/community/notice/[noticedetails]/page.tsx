...
import NoticeDeleteButton from "@/app/ui/community/noticedeletebutton";
...
export default async function NoticeDetails({ params, searchParams }: BoardArg) {
  ...
  return (
    <div>
      <main className="mx-auto p-4 pt-10 pb-10 sm:w-3/5">
        <div className="text-3xl text-center mb-6">{boardtitle}</div>
        <BoardArticle post={postdata} />
      </main>
      <div className="flex w-full justify-center p-2">
        {postdata && <NoticeDeleteButton tablename={tablename} postindex={postindex} nextrouter={nextrouter} postdataid={postdataid} />}
      </div>
    </div>
  );
}
