    ...
    <div className="sm:flex sm:justify-around sm:p-6 ">
      ...
      {eventDetails ? (
        <article className="p-2 sm:p-10 sm:w-2/5 sm:flex sm:justify-start sm:flex-col sm:mt-4">
          ...
          <div className="flex flex-col sm:text-base sm:mt-4 mt-2">
            <div>클래스 수강료는 현장결제 및 계좌이체만 가능한 점 양해 부탁드립니다. </div>
            <div>
              클래스 날짜 및 시간은 당월 인원의 스케쥴에 따라 달라질수 있습니다.
            </div>
          </div>
          <div className="sm:mt-10 sm:w-full flex justify-center items-center sm:h-20 border border-black sm:px-10 mt-5 h-10 bg-black">
            <Link href="/community/classregister " className="w-full h-full rounded-md">
              <button className="sm:text-1xl text-white w-full h-full text-xs">클래스신청 바로가기</button>
            </Link>
          </div>
        </article>
      ) : (
